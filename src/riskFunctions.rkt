...
roll: [Listof die(attack)] [Listof die(defend)] -> roll
    - Contains two lists, one of a list of attacker dice and another of defender dice.
|#
...
(define (numberbar armies selected)
  (above
   (beside (overlay (textc "0" 12  "black")
                    (rectangle (/ SLIDER-WIDTH 5) 0 "solid" (make-color 0 0 0 0)))
           (overlay (textc (number->string (round (/ armies 4))) 12 "black")
                    (rectangle (/ SLIDER-WIDTH 5) 0 "solid" (make-color 0 0 0 0)))
           (overlay (textc (number->string (round (/ armies 2))) 12 "black")
                    (rectangle (/ SLIDER-WIDTH 5) 0 "solid" (make-color 0 0 0 0)))
           (overlay (textc (number->string (round (/ (* 3 armies) 4))) 12 "black")
                    (rectangle (/ SLIDER-WIDTH 4) 0 "solid" (make-color 0 0 0 0)))
           (overlay (textc (number->string armies) 12 "black")
                    (rectangle (/ SLIDER-WIDTH 4) 0 "solid" (make-color 0 0 0 0)))
           )
...
(define (create-slider armies x y selected)
  (make-slider (above (place-image SLIDER-HEAD
                                   x
                                   y
                                   SLIDER-BAR)
...
(define (x/o x)
  (- x (/ (- display-w (image-width BOARD)) 2)))
...
(define (attack-phase model x y event)
  (cond [(won-game? (system-territory-list model)
         ...
         ;Checks to see if a player has won the game, the first condition.
         (struct-copy system model
                      [screen "victory"]
        ...
        [(and (equal? event "button-down")
         ...
         (struct-copy system model
                      [territory-selected (tooltip (x/o x) y model)]
                      [territory-attacking (tooltip (x/o x) y model)]
                      [x (- x (/ (- display-w (image-width BOARD)) 2))]
                      [y y]
