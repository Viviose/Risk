...
(define (keyer model key)
  (begin  (cond [(equal? key "a") (check-readiness model)]
...
(define (write-list-to-file model)
  (begin
    (define out (open-output-file "Map-Positions/map-positions.txt" #:exists 'can-update))
...
(define (make-model-ellipse model)
  (ellipse (abs (- (posn-x (custom-ellipse-point-2 (masterGuy-current-ellipse model))) (posn-x (custom-ellipse-point-1 (masterGuy-current-ellipse model)))))
                (* 2 (- (posn-y (custom-ellipse-point-1 (masterGuy-current-ellipse model))) (posn-y (custom-ellipse-point-3 (masterGuy-current-ellipse model)))))
                "outline"
                "red")
  )
...
(define (drawer model)
   (cond [(third-ready? model) 
           (overlay/align "left" "top"
           (place-model-ellipse model)
